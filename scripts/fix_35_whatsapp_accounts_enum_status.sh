...
FIX_LOG_FILE="${LOGS_DIR}/fix_35_whatsapp_accounts_enum_status.log"
...
echo "== Correcao Etapa 35: enum WhatsappAccountStatus =="
...
echo "Regravando whatsapp-accounts.service.ts com enum Prisma..."
...
LOCAL_PHONE_NUMBER_ID="local_phone_fix35_${STAMP}"
LOCAL_CREATE_PAYLOAD="$(node -e "console.log(JSON.stringify({wabaId:'local_waba_fix35', phoneNumberId:process.argv[1], displayPhoneNumber:'+55 21 99999 3535', verifiedName:'Conta Local Fix 35', accessToken:'token_local_fix_35', status:'pending'}))" "${LOCAL_PHONE_NUMBER_ID}")"
...
LOCAL_UPDATE_PAYLOAD="$(node -e "console.log(JSON.stringify({verifiedName:'Conta Local Fix 35 Atualizada', status:'active'}))")"
...
DOMAIN_PHONE_NUMBER_ID="domain_phone_fix35_${STAMP}"
DOMAIN_CREATE_PAYLOAD="$(node -e "console.log(JSON.stringify({wabaId:'domain_waba_fix35', phoneNumberId:process.argv[1], displayPhoneNumber:'+55 21 98888 3535', verifiedName:'Conta Dominio Fix 35', accessToken:'token_domain_fix_35', status:'pending'}))" "${DOMAIN_PHONE_NUMBER_ID}")"
...
if ! grep -q "Conta Dominio Fix 35" "${DOMAIN_CREATE_LOG}"; then
...
Etapa: 35
Acao: Correcao enum WhatsappAccountStatus
...
echo ""
echo "== Etapa 35 corrigida e concluida com sucesso =="
echo ""
echo "Resumo:"
