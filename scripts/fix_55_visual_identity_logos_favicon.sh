...
FIX_LOG_FILE="${LOGS_DIR}/fix_55_visual_identity_logos_favicon.log"
...
echo "== Fix Etapa 55: Identidade visual com logos e favicon =="
...
echo "Copiando logos para public..."
...
if [ ! -s "${PUBLIC_ASSETS_DIR}/chatbot_logo.png" ]; then
  echo "ERRO: chatbot_logo.png nao foi publicado."
  exit 1
fi

if [ ! -s "${PUBLIC_DIR}/favicon.png" ]; then
  echo "ERRO: favicon.png nao foi publicado."
  exit 1
fi

echo "Regravando index.html seguro..."
...
echo "Regravando Sidebar.tsx seguro..."
...
echo "Removendo trechos obviamente corrompidos se existirem..."
...
for path in files:
    ...
    text = path.read_text()
    text = text.replace('    text = text.replace('target="_blank"', '')
    text = text.replace('rel="noopener noreferrer"', '')
    text = text.replace('class="fai-ChatInputEntity__text ___6erqso0 fyind8e f1tx3yz7 f1deo86v f1eh06m1 f1iescvh"', '')
    path.write_text(text)
PY

echo "Adicionando CSS seguro da identidade visual..."
...
A Etapa 55 foi corrigida com regravacao segura dos arquivos visuais para remover corrupcao de aspas e HTML injetado no script anterior.
...
- logs/setup_55.log
- logs/fix_55_visual_identity_logos_favicon.log
...
Etapa: 55
Acao: Correcao completa da identidade visual
Data: $(date '+%Y-%m-%d %H:%M:%S')
Status: Concluido
DOC
