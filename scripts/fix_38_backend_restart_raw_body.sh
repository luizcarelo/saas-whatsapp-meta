...
FIX_LOG_FILE="${LOGS_DIR}/fix_38_backend_restart_raw_body.log"
NPM_BODY_PARSER_LOG="${LOGS_DIR}/fix_38_install_body_parser.log"
NPM_TYPES_BODY_PARSER_LOG="${LOGS_DIR}/fix_38_install_types_body_parser.log"
...
echo "== Correcao Etapa 38: backend restart e raw body =="
...
echo "Instalando body-parser e tipos..."

cd "${BACKEND_DIR}"

npm install body-parser 2>&1 | tee "${NPM_BODY_PARSER_LOG}"
npm install --save-dev @types/body-parser 2>&1 | tee "${NPM_TYPES_BODY_PARSER_LOG}"

cd "${BASE_DIR}"

echo "Recriando raw-body.middleware.ts sem import de express..."
...
echo "Atualizando main.ts para usar body-parser..."
...
echo "Atualizando meta-webhooks.controller.ts..."
...
docker compose config > "${LOGS_DIR}/fix_38_docker_compose_config.log"
...
VERIFY_CHALLENGE="challenge_38_fix_${STAMP}"
      ...
      "changes": [
        {
          ...
          "value": {
            ...
            "metadata": {
              ...
              "phone_number_id": "phone_signature_38_fix_${STAMP}"
            },
            "contacts": [
              {
                "wa_id": "552188883838",
                "profile": {
                  "name": "Contato Assinatura Etapa 38 Fix"
                }
              }
            ],
            "messages": [
              {
                "from": "552188883838",
                "id": "wamid.signature.fix.etapa38.${STAMP}",
                "timestamp": "1760000000",
                "type": "text",
                "text": {
                  "body": "Mensagem assinada etapa 38 fix"
                }
              }
            ]
          }
...
Foi ajustada a captura de raw body para usar body-parser.

Tambem foi adicionado aguardo ativo do backend antes das validacoes.
...
- logs/fix_38_backend_restart_raw_body.log
- logs/setup_38.log
...
Etapa: 38
Acao: Correcao backend restart raw body
