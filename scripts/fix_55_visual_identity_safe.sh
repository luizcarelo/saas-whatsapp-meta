...
FIX_LOG_FILE="${LOGS_DIR}/fix_55_visual_identity_safe.log"
...
echo "== Fix seguro Etapa 55: Identidade visual com logos e favicon =="
...
if [ ! -f "${LOGO_COMPANY}" ]; then
  echo "ERRO: logo compacto ausente: favicon.png"
  exit 1
fi
...
if [ ! -s "${PUBLIC_ASSETS_DIR}/chatbot_logo.png" ]; then
  echo "ERRO: chatbot_logo.png nao publicado."
  exit 1
fi

if [ ! -s "${PUBLIC_DIR}/favicon.png" ]; then
  echo "ERRO: favicon.png nao publicado."
  exit 1
fi

echo "Regravando index.html e Sidebar.tsx sem caracteres corrompiveis..."
...
{lt}html lang="pt-BR"{gt}
  {lt}head{gt}
    {lt}meta charset="UTF-8" /{gt}
    {lt}meta name="viewport" content="width=device-width, initial-scale=1.0" /{gt}
    {lt}meta name="theme-color" content="#0757c8" /{gt}
    {lt}link rel="icon" type="image/png" href="/favicon.png" /{gt}
    {lt}title{gt}LH Solucao Chat Bot{lt}/title{gt}
  {lt}/head{gt}
  {lt}body{gt}
    {lt}div id="root"{gt}{lt}/div{gt}
    {lt}script type="module" src="/src/main.tsx"{gt}{lt}/script{gt}
  {lt}/body{gt}
{lt}/html{gt}
"""

Path("apps/frontend/index.html").write_text(index)

sidebar = f"""import {{ NavLink }} from 'react-router-dom';

export function Sidebar() {{
  return (
    {lt}aside className="sidebar"{gt}
      {lt}div className="sidebar-header"{gt}
        {lt}img
        ...
        {lt}div{gt}
          ...
          {lt}span{gt}Chat Bot Meta{lt}/span{gt}
        {lt}/div{gt}
      {lt}/div{gt}

      {lt}nav className="sidebar-nav"{gt}
        {lt}NavLink to="/app/dashboard"{gt}Dashboard{lt}/NavLink{gt}
        {lt}NavLink to="/app/contacts"{gt}Contatos{lt}/NavLink{gt}
        {lt}NavLink to="/app/conversations"{gt}Conversas{lt}/NavLink{gt}
        {lt}NavLink to="/app/whatsapp-accounts"{gt}WhatsApp{lt}/NavLink{gt}
        {lt}NavLink to="/app/meta-settings"{gt}Meta{lt}/NavLink{gt}
        {lt}NavLink to="/app/audit"{gt}Auditoria{lt}/NavLink{gt}
        {lt}NavLink to="/app/audit-real-run"{gt}Higienizacao real{lt}/NavLink{gt}
        {lt}NavLink to="/app/audit-real-history"{gt}Historico higiene{lt}/NavLink{gt}
        {lt}NavLink to="/app/profile"{gt}Perfil{lt}/NavLink{gt}
      {lt}/nav{gt}
    {lt}/aside{gt}
  );
}}
...
Path("apps/frontend/src/components/layout/Sidebar.tsx").write_text(sidebar)
PY

echo "Adicionando CSS visual seguro..."
...
echo "Validando ausencia de HTML injetado por chat..."
...
if grep -R "target=\"_blank\"" \
  "${FRONTEND_DIR}/index.html" \
  "${FRONTEND_DIR}/src/components/layout/Sidebar.tsx" \
  "${FRONTEND_DIR}/src/styles.css"
then
  echo "ERRO: atributo target indevido encontrado."
  exit 1
fi
...
Etapa: 55
Acao: Correcao segura da identidade visual
Data: $(date '+%Y-%m-%d %H:%M:%S')
Status: Concluido
DOC

echo ""
echo "== Etapa 55 corrigida e concluida com sucesso =="
echo ""
echo "Resumo:"
