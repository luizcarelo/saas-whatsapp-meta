...
echo "== Etapa 22: Prisma ORM e conexao real com PostgreSQL =="
...
echo "Instalando Prisma no backend..."

cd "${BACKEND_DIR}"

npm install @prisma/client 2>&1 | tee "${NPM_LOG}"
npm install --save-dev prisma 2>&1 | tee -a "${NPM_LOG}"

cd "${BASE_DIR}"

echo "Criando prisma/schema.prisma..."
...
echo "Atualizando DatabaseService com consulta real..."
...
echo "Ajustando backend.Dockerfile para Prisma..."
...
- npm install prisma como dev dependency
- npx prisma generate
- npm run typecheck
- npm run build
...
Endpoint local:
    ...
    http local api v1 health

Endpoint dominio:

    https bot lhsolucao api v1 health
...
echo ""
echo "== Etapa 22 concluida com sucesso =="
echo ""
echo "Resumo:"
