...
FIX_LOG_FILE="${LOGS_DIR}/fix_38_install_platform_express.log"
NPM_INSTALL_LOG="${LOGS_DIR}/fix_38_install_platform_express_npm.log"
...
echo "== Correcao Etapa 38: instalar @nestjs/platform-express =="
...
echo "Instalando @nestjs/platform-express na mesma versao do @nestjs/core..."

cd "${BACKEND_DIR}"

NEST_CORE_VERSION="$(node -e "const pkg=require('./package.json'); const deps=Object.assign({}, pkg.dependencies || {}, pkg.devDependencies || {}); const v=deps['@nestjs/core'] || ''; console.log(v.replace(/^[~^]/, ''))")"

if [ -z "${NEST_CORE_VERSION}" ]; then
  echo "ERRO: versao de @nestjs/core nao encontrada."
  exit 1
fi

npm install "@nestjs/platform-express@${NEST_CORE_VERSION}" 2>&1 | tee "${NPM_INSTALL_LOG}"
...
echo "Garantindo main.ts com rawBody nativo do Nest..."
...
echo "Mantendo raw-body.middleware.ts apenas como tipo auxiliar..."
...
echo "Garantindo controller usando request.rawBody..."
...
docker compose config > "${LOGS_DIR}/fix_38_install_platform_express_docker_compose_config.log"
...
if [ "${BACKEND_READY}" != "true" ]; then
  ...
  docker compose logs --tail=240 backend 2>&1 | tee "${BACKEND_CRASH_LOG}"
  exit 1
fi
...
VERIFY_CHALLENGE="challenge_38_platform_${STAMP}"
      ...
      "changes": [
        {
          ...
          "value": {
            ...
            "metadata": {
              ...
              "phone_number_id": "phone_signature_38_platform_${STAMP}"
            },
            "contacts": [
              {
                "wa_id": "552188883838",
                "profile": {
                  "name": "Contato Assinatura Etapa 38 Platform"
                }
              }
            ],
            "messages": [
              {
                "from": "552188883838",
                "id": "wamid.signature.platform.etapa38.${STAMP}",
                "timestamp": "1760000000",
                "type": "text",
                "text": {
                  "body": "Mensagem assinada etapa 38 platform"
                }
              }
            ]
          }
...
Etapa: 38
Acao: Instalacao @nestjs/platform-express
