...
FIX_LOG_FILE="${LOGS_DIR}/fix_24_seed_env_safe_v2.log"
...
echo "== Correcao Etapa 24 v2: seed seguro =="
...
echo "Atualizando variaveis de seed..."

set_env_value "SEED_TENANT_NAME" "LH Solucao" "${BASE_DIR}/.env.example"
set_env_value "SEED_TENANT_DOCUMENT" "" "${BASE_DIR}/.env.example"
set_env_value "SEED_ADMIN_NAME" "Administrador" "${BASE_DIR}/.env.example"
set_env_value "SEED_ADMIN_EMAIL" "admin@lhsolucao.com.br" "${BASE_DIR}/.env.example"
set_env_value "SEED_ADMIN_PASSWORD" "change_me_admin_password" "${BASE_DIR}/.env.example"
...
echo "Garantindo bcryptjs..."

cd "${BACKEND_DIR}"

if ! node -e "require('bcryptjs')" >/dev/null 2>&1; then
  npm install bcryptjs 2>&1 | tee "${LOGS_DIR}/setup_24_backend_npm.log"
fi

cd "${BASE_DIR}"

echo "Recriando seed.js limpo..."
...
echo "Validando se ainda ha caractere proibido nos arquivos criticos..."

BAD_CHAR="$(printf '\052')"

if grep -n "${BAD_CHAR}" "${BACKEND_DIR}/prisma/seed.js"; then
  echo "ERRO: caractere proibido encontrado no seed.js."
  exit 1
fi

if grep -R "<a href" "${BACKEND_DIR}/prisma/seed.js" "${BASE_DIR}/.env" "${BASE_DIR}/.env.example"; then
  echo "ERRO: HTML indevido encontrado."
  exit 1
fi

echo "Executando seed inicial sem source do env..."
