...
LOG_FILE="${LOGS_DIR}/setup_39.log"
FRONTEND_TYPECHECK_LOG="${LOGS_DIR}/setup_39_frontend_typecheck.log"
FRONTEND_BUILD_LOG="${LOGS_DIR}/setup_39_frontend_build.log"
DOCKER_BUILD_LOG="${LOGS_DIR}/setup_39_frontend_docker_build.log"
DOCKER_UP_LOG="${LOGS_DIR}/setup_39_frontend_docker_up.log"
DOMAIN_LOGIN_LOG="${LOGS_DIR}/setup_39_auth_login_domain.log"
DOMAIN_CONVERSATIONS_LIST_API_LOG="${LOGS_DIR}/setup_39_conversations_list_domain.log"
DOMAIN_CONVERSATIONS_CREATE_API_LOG="${LOGS_DIR}/setup_39_conversations_create_domain.log"
DOMAIN_CONVERSATIONS_MESSAGE_API_LOG="${LOGS_DIR}/setup_39_conversations_message_domain.log"
DOMAIN_CONVERSATIONS_PAGE_LOG="${LOGS_DIR}/setup_39_domain_conversations_page.log"
DOMAIN_DASHBOARD_LOG="${LOGS_DIR}/setup_39_domain_dashboard.log"
DOC_FILE="${DOCS_DIR}/FRONTEND_MESSAGE_STATUS.md"
...
echo "== Etapa 39: Processamento de status de mensagens no frontend =="
...
CONVERSATION_PHONE="5521339${STAMP}"
CONVERSATION_PAYLOAD="$(node -e "console.log(JSON.stringify({name:'Conversa Status Etapa 39', phone:process.argv[1], initialMessage:'Mensagem inbound para status etapa 39'}))" "${CONVERSATION_PHONE}")"
...
if ! grep -q "Conversa Status Etapa 39" "${DOMAIN_CONVERSATIONS_CREATE_API_LOG}"; then
...
MESSAGE_PAYLOAD="$(node -e "console.log(JSON.stringify({body:'Resposta outbound pending etapa 39'}))")"
...
if ! grep -q "Resposta outbound pending etapa 39" "${DOMAIN_CONVERSATIONS_MESSAGE_API_LOG}"; then
...
echo "Revalidando conversations.service.ts..."
...
echo "Atualizando ConversationsPage com status visual..."
...
echo "Adicionando estilos de status de mensagens..."
...
.message-status-summary article {
  background: #ffffff;
...
.thread-message-footer span:first-child {
  font-size: 12px;
  opacity: 0.75;
}
...
.thread-message.outbound .message-status-badge {
...
echo "Gerando documentacao da Etapa 39..."
...
- Etapa 01 ate Etapa 39 concluidas
...
cat > "${LOG_FILE}" <<DOC
Etapa: 39
Acao: Processamento de status de mensagens no frontend
...
echo ""
echo "== Etapa 39 concluida com sucesso =="
echo ""
echo "Resumo:"
