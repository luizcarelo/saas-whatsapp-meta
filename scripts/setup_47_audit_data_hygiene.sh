...
echo "== Etapa 47: Higienizacao de dados de auditoria antigos =="
...
echo "Atualizando backend de auditoria com higienizacao..."
...
echo "Garantindo modulo operacional no app.module.ts..."
...
echo "Atualizando frontend com higienizacao..."
...
if "previewAuditHygieneRequest" not in text:
    text = text.replace(
        "listAuditWebhooksRequest",
        "listAuditWebhooksRequest,\n  previewAuditHygieneRequest,\n  runAuditHygieneRequest"
    )

if "AuditHygieneResult" not in text:
    text = text.replace(
        "AuditWebhookItem",
        "AuditHygieneResult,\n  AuditWebhookItem"
    )

if "const [hygieneDays, setHygieneDays]" not in text:
    text = text.replace(
...
if "async function handlePreviewHygiene" not in text:
    marker = "  async function handleExport(resource: string, format: string) {"
    methods = """
...
if "audit-hygiene-panel" not in text:
    marker = "      <div className=\"audit-summary-grid\">"
    panel = """      <section className=\"audit-hygiene-panel\">
        <div>
...
echo ""
echo "== Etapa 47 concluida com sucesso =="
echo ""
echo "Resumo:"
