...
FIX_LOG_FILE="${LOGS_DIR}/fix_18.log"
DEPENDENCIAS_DOC="${DOCS_DIR}/DEPENDENCIAS_BASE.md"

echo "== Correcao Etapa 18: dependencias e typecheck =="
...
echo "Regravando apps/backend/tsconfig.json sem baseUrl..."
...
if grep -n "&1 | tee "${LOGS_DIR}/setup_18_backend_npm_install.log"
else
  echo "package-lock do backend encontrado."
fi
...
if [ ! -d "${FRONTEND_DIR}/node_modules" ]; then
  ...
  echo "node_modules do frontend encontrado. Rodando npm install para sincronizar lockfile..."
  npm install 2>&1 | tee "${LOGS_DIR}/setup_18_frontend_npm_install.log"
fi
...
echo ""
echo "== Etapa 18 corrigida e concluida com sucesso =="
echo ""
echo "Resumo:"
